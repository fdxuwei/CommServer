...
typedef boost::function<void (const muduo::net::TcpConnectionPtr &conn, const std::string &servName)> ClientConnectionCallback;
...
public:
	...
	{ messageCallback_ = cb; }
	void removeTcpClient(const std::string &servName, const std::string &ip, int port);
	void removeTcpClientIfNotRetry(const std::string &servName, const std::string &ip, int port);
private:
	//
	typedef std::map<std::string, TcpClientPtr> TcpClientMap;
	//
	void connectInLoop(const std::string &servName, const std::string &ip, int port);
	void removeTcpClientInLoop(const std::string &servName, const std::string &ip, int port);
	void removeTcpClientIfNotRetryInLoop(const std::string &servName, const std::string &ip, int port);
